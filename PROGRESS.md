...
3. **Firebase 프로젝트 설정 — 진행 중** (아래 §5 참고)
...
### 4.1 지금 하던 것 — Firebase 계정/키 세팅 (①1번부터 순서대로 진행 중)
1. ✅ Firebase 프로젝트 생성 완료
2. ✅ Firestore Database 활성화 완료
3. ⏭️ Storage — **건너뛰기로 결정** (위 §2 참고)
4. ✅ 웹 앱 등록 + `firebaseConfig` 확보 완료 (아래 §5 참고)
5. ⬜ **서비스 계정 키(Admin SDK) 발급 — 진행 중, 아직 미완료.** 사용자에게 안내한 절차: Firebase 콘솔 → 프로젝트 설정 → "서비스 계정" 탭 → "새 비공개 키 생성" → 다운로드된 JSON 파일을 `c:\Users\USER\Desktop\golf-dashboard\` 폴더에 옮겨두면 다음 세션에서 그 파일을 직접 읽어 `.env.local`에 안전하게(커밋 안 되게) 설정 예정. **주의: 이 파일이 실제로 폴더에 도착하면 절대 커밋하지 말 것** — `.gitignore`에 `*firebase-adminsdk*.json` 등 패턴을 미리 추가해뒀음.
...
### 4.3 코드 구현 (서비스 계정 키 받으면 바로 착수 가능)
- Next.js 프로젝트 스캐폴딩 (예: `web/` 서브폴더에)
- Firebase 클라이언트 SDK 연결 (`firebaseConfig` 사용, 읽기 전용)
- Firebase Admin SDK 연결 (서비스 계정 키 사용, 서버 API Route에서만 사용 — 쓰기 전담)
- Firestore 데이터 스키마 구현 (PRD.md §4)
- Firestore 보안 규칙 작성 (읽기 공개 / 쓰기 전부 차단, Admin SDK는 규칙 우회하므로 문제 없음)
- 엑셀 파싱 로직 구현 (`xlsx` 라이브러리, PRD.md §2 구조 분석 결과 활용)
- 업로드 페이지(`/upload`, PIN) 실제 구현
- 대시보드 실제 구현 (프로토타입 → 실제 데이터 연동)
