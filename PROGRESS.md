...
- ⬜ **다음으로 할 것**: 그린피 단가 입력 폼(현재 `/upload`에 없음 — 관리자가 직접 Firestore에 넣거나 입력 화면을 새로 만들어야 함), 기상청 API 연동(`weatherCache` 채우기), 실제 "하루 단위" 매출 파일로 `dailySales` 파서 재검증, 디자인 다듬기(나중으로 보류 중)
