...
- ✅ **엑셀 파싱 로직 3종 작성 + 실제 샘플 파일로 검증 완료** (`web/src/lib/parsers/`)
  ...
  - `dailySales.ts` — 일일영업집계 .xls 중 "매출집계" 서브테이블만 파싱 (그린피/카트료/대여료/식음/상품/기타매출 6항목 + 합계). **⚠️ 실제 "하루 단위" 재추출 샘플이 아직 없어서, 월간 샘플로 구조만 검증함** (합계 검산 100% 일치 확인) — 실제 운영 전 현장에서 받은 진짜 1일치 파일로 재검증 필요. 디버깅 중 실제 버그 발견·수정: "그린피"라는 헤더가 무관한 다른 서브테이블(그린피 단가별 팀수표)에도 존재해서 엉뚱한 값(0)을 읽어오던 것 — "카트료"(이 서브테이블에만 존재하는 유일 텍스트)를 기준점으로 삼아 가장 가까운 헤더를 선택하도록 수정.
- ⬜ **다음으로 할 것**: `/upload` 페이지 실제 구현 (PIN 입력 + 드래그앤드롭 업로드 + 위 파서 호출 + Firestore 쓰기 API Route), 대시보드 실제 구현 (프로토타입 → 실데이터 연동)
