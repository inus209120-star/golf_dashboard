...
최종 업데이트: 2026-09-11 (기상청 날씨 API 연동 완료)
...
- ⬜ **다음으로 할 것**: 실제 "하루 단위" 매출 파일로 `dailySales` 파서 재검증, `UPLOAD_PIN` 실제 값으로 변경(현재 `1234` 임시값), 디자인 다듬기(나중으로 보류 중)
...
- Vercel 계정 준비 (배포는 사용자가 명시적으로 보류 요청 — 지금은 로컬호스트로만 확인 중)
