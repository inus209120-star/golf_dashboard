...
import type { DailyVisitorDoc, UploadLogDoc } from "@/types/firestore";

// Each 종합영업일보 file covers exactly one day, so the upload page lets
// field staff select many files at once (one per day) - the client parses
// all of them and posts the resulting docs here as a single batch.
export async function POST(req: Request) {
  const body = await req.json();
  const pinError = checkPin(body?.pin);
  if (pinError) return pinError;

  const docs: DailyVisitorDoc[] = Array.isArray(body?.docs) ? body.docs : [];
  ...
  for (const doc of docs) {
    batch.set(adminDb.collection(COLLECTIONS.dailyVisitors).doc(doc.date), doc);
  }
  await batch.commit();
