...
import type { CashFlowDoc, UploadLogDoc } from "@/types/firestore";
...
export async function POST(req: Request) {
  ...
  const docs: CashFlowDoc[] = Array.isArray(body?.docs) ? body.docs : [];
  ...
  for (const doc of docs) {
    batch.set(adminDb.collection(COLLECTIONS.cashFlow).doc(doc.date), doc);
  }
  await batch.commit();
